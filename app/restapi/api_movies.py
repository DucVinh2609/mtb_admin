...
class apiMovieDetailDate(Resource):
  def get(self, id):
			conn = mysql.connect()
			cursor = conn.cursor(pymysql.cursors.DictCursor)
			sql="SELECT id id, movie_id movie_id, room_id room_id, showtime showtime from showwings WHERE movie_id=%s GROUP BY showtime ORDER BY showtime DESC"
			data=(id)
...
class apiMovieDetailTime(Resource):
  def get(self, id):
			...
			# sql=""
			# data=(id)
			cursor.execute("SELECT id id, movie_id movie_id, room_id room_id, showtime showtime from showwings WHERE movie_id=%s",id)
			rows = cursor.fetchall()
			resp = jsonify(rows)
			resp.status_code = 200
			return resp
